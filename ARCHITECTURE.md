...
4. **持久化可靠**: 原子写入+自动备份机制
5. **可测试性**: 依赖注入+接口抽象
6. **Interleaved Thinking** (v0.2.4+): 三阶段执行模型，支持思考与工具调用交错
...
### 1.3 Interleaved Thinking 架构 (v0.2.4+)

Interleaved Thinking 采用三阶段执行模型：

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Interleaved Thinking 执行流程                     │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   ┌──────────┐      ┌──────────────┐      ┌────────────┐           │
│   │ thinking │ ───► │  tool_call   │ ───► │  analysis  │           │
│   │   🧠     │      │     🔧       │      │     📊     │           │
│   └──────────┘      └──────────────┘      └────────────┘           │
│        │                   │                    │                   │
│        │                   ▼                    │                   │
...
├── __init__.py
├── thought.py               # 思考步骤模型
├── thinking_session.py      # 思考会话模型（含统计信息）
├── tool_call.py             # 工具调用模型 (v0.2.4+)
├── config.py                # 配置模型 (v0.2.4+)
├── task.py                  # 任务模型
└── template.py              # 模板模型
```
...
v0.2.4+ 新增 Interleaved Thinking 字段：
...
class Thought(BaseModel):
    ...
    tool_calls: list[str] = Field(
        ...
        description="关联的工具调用记录ID列表（1:N 映射）"
    )
...
class SessionStatistics(BaseModel):
    ...
    phase_distribution: dict[str, int] = {"thinking": 0, "tool_call": 0, "analysis": 0}
