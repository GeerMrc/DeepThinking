...
**测试目的**: 验证AI能使用sequential_thinking工具进行常规思考

**对话提示词**:
> "使用sequential_thinking工具，分析Python中列表和元组的区别。请进行3步思考。"
...
**对话提示词**:
> "使用sequential_thinking工具进行思考。第一步：假设MySQL是最优数据库选择。第二步：修订你的第一步思考，考虑PostgreSQL可能在ACID合规性上更好。"
...
**对话提示词**:
> "使用sequential_thinking分析微服务架构。第一步：分析优势。第二步：从第一步分支，专门分析数据一致性的挑战。"
...
**对话提示词**:
> "创建一个新任务，标题是'设计数据库Schema'，优先级为P1，描述是'设计用户和订单表结构'。"
...
**对话提示词**:
> "请帮我完成一个完整的思考流程：1.创建一个名为'技术选型'的会话；2.使用sequential_thinking分析选择PostgreSQL还是MySQL；3.考虑修订你的分析；4.最后将会话导出为Markdown格式。"

**预期AI行为**:
1. 调用 `create_session`
2. 调用 `sequential_thinking`（常规思考）
3. 调用 `sequential_thinking`（修订思考）
4. 调用 `export_session`
...
1. 调用 `create_task`
2. 调用 `create_session`
3. 调用 `sequential_thinking`（一次或多次）
